...
import { User, Shield, Search, LogOut } from "lucide-react";
...
export default async function AdminRootLayout({ children }: { children: ReactNode }) {
    ...
    return (
        <div className="min-h-screen flex bg-gray-50 dark:bg-gray-950">
            {/* Sidebar */}
            <aside className="w-64 bg-gray-900 text-white flex flex-col hidden md:flex shadow-xl z-20">
                ...
                <nav className="flex-1 overflow-y-auto py-6 space-y-1 px-3">
                    <Link href="/admin/users"
                        className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white hover:bg-white/10 transition-all font-medium">
                        <User size={18} />
                ...
                <div className="p-4 border-t border-gray-800">
                    {/* Logout logic could go here, for now just a link or visual */}
                    <div className="flex items-center gap-3 text-gray-400 hover:text-white cursor-pointer px-3 py-2 transition-colors">
                        <LogOut size={18} />
            ...
            <div className="flex-1 flex flex-col min-w-0">
                {/* Top Navigation Bar */}
                <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 h-16 flex items-center justify-between px-6 lg:px-8 shadow-sm">
